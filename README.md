...
Beat your March Madness pool with statistical bracket optimization. madlab scrapes live game data from ESPN, fits a team-strength model, and runs thousands of simulations to find the bracket most likely to win your specific pool — not just pick favorites.
...
The app auto-loads the current year's bracket, lets you tune pool size, scoring scheme, and optimization criterion, and streams live progress during the optimization. The bracket renders as a card-based SVG with hover-to-trace team paths and an animated score distribution histogram.
...
# Load bundled 2026 data
bracket = load_bracket("men", 2026)   # 64 team IDs in seed order
games   = load_games("men", 2026)     # ~6000 regular-season games
...
if missing:
    ...
    save_bracket(bracket, "men", 2027)
```

`prep_data` concurrently scrapes all ~350 team schedules using async HTTP, so the full season ingest typically takes under a minute.

### Step 3 — Use the new data

```python
games       = load_games("men", 2027)
bracket     = load_bracket("men", 2027)
prob_matrix = bradley_terry(games)
```
...
- `ot_margin` — value assigned to OT games instead of 0 (default `0.0`)
...
Adds +¾ to the log-odds of the conditional win probability for each listed team. Pass via `home_teams=` in `sim_bracket` or `pool_bias=` in `find_bracket`.
...
Pre-scraped data is included for the following years:
